...
int choiceChecking(std::string userChoice) {
  int return_Value;
  std::cout << std::endl;
  while (userChoice.at(0) != '1' && userChoice.at(0) != '2' &&
         userChoice.at(0) != '3') {
...
// Function to check the user switch choice. The user can enter 'yes' or 'no,
// the first letter will be taken into account.
char swtichChoiceChecking(std::string switchChoice) {
  std::cout << std::endl;
  while (tolower(switchChoice.at(0)) != 'y' &&
         tolower(switchChoice.at(0)) != 'n') {
    std::cout << "I did not understant that. Please enter 'y' for yes or 'n' "
                 "for no: ";
...
// Function to return a door that does not have the prize and is not the user's
// selection
...
void LiveGame() {
  ...
  std::cout << std::endl;
  std::cout << "Ok let's calm down a bit. I want to tell you that behind door "
            << OtherDoorFrom(fixed_User_Door, prize_Door) << std::endl;
  std::cout << "there's one of the two cases for a Macbook, and you chose "
            << fixed_User_Door << "." << std::endl;
  std::cout << "Your choice might be right and you could win the latest "
               "Macbook pro, but who knows what could happen."
            << std::endl;
  std::cout << "So, I will give you the chance to switch from door "
            << fixed_User_Door << " to door "
            << SwitchDoorTo(fixed_User_Door, prize_Door) << std::endl;
  ...
  if (fixed_Switch_Choice == 'y') {
    ...
    if (new_User_Door == prize_Door) {
      switch_wins += 1;
      std::cout << "You were wise by switching doors. You just won a Macbook "
                   "Pro! Congratulations!"
                << std::endl;

    } else {
      std::cout << "Well, you won the case, you just need the Macbook now. "
                   "Good luck next time!"
                << std::endl;
    }
    ...
    if (new_User_Door == prize_Door) {
      not_switch_wins += 1;
      std::cout << "You were wise by not switching doors. You just won a "
                   "Macbook Pro! Congratulations!"
                << std::endl;
...
void simulatedGame() {
  ...
  std::cout << simulations
            << " simulations done. Results will display at the end of the game."
            << std::endl
            << std::endl;
  return;
}
...
void endGame() {
  ...
  std::cout << std::left << std::setw(14) << "Strategy" << std::setw(14)
            << "Games" << std::setw(14) << "Wins" << std::setw(14) << "Win Rate"
            << std::endl;
  std::cout << line << std::endl;
  std::cout << std::fixed << std::setprecision(2) << std::setw(15) << "Switch"
            << switch_games << std::setw(13) << "" << switch_wins
            << std::setw(13) << "" << switch_Win_Rate << "%" << std::endl;
  std::cout << line << std::endl;
  std::cout << std::fixed << std::setprecision(2) << std::setw(15)
            << "Not Switch" << not_switch_games << std::setw(13) << ""
            << not_switch_wins << std::setw(13) << "" << not_switch_Win_Rate
            << "%" << std::endl;
  std::cout << line << std::endl;
  std::cout << std::fixed << std::setprecision(2) << std::setw(15) << "Total"
            << total_Games << std::setw(13) << "" << total_Wins << std::setw(13)
            << "" << total_Win_Rate << "%" << std::endl;
  return;
}
